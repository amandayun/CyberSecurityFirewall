...
class final_topo(Topo):
  def build(self):
    ...
    self.addLink(s5, h50, port1=8, port2=0)
    self.addLink(s5, h60, port1=9, port2=0)

    self.addLink(s2, h30, port1=8, port2=0)
    self.addLink(s2, h40, port1=9, port2=0)
    self.addLink(s6, h70, port1=8, port2=0)
    self.addLink(s6, h80, port1=9, port2=0)

    #linking to core
    self.addLink(s3, s1, port1=1, port2=1)
    self.addLink(s3, s2, port1=2, port2=1)
    self.addLink(s3, s5, port1=3, port2=1)
    self.addLink(s3, s6, port1=4, port2=1)
    self.addLink(s3, s4, port1=5, port2=1)

    self.addLink(s3, h_trust, port1=6, port2=0)
    self.addLink(s3, h_untrust, port1=7, port2=0)
    self.addLink(s4, h_server, port1=8, port2=0)
